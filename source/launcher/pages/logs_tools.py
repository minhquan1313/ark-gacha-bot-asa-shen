...
class LogsToolsPagesMixin:
    ...
    def _update_page(self):
        ...
        changelog_text = QLabel("Open this page to check for the newest release.")
    ...
    def _automatic_update_check(self):
        ...
        if getattr(self, "update_auto_check_enabled", True):
    ...
    def _on_update_check_finished(self, result: UpdateCheckResult, automatic):
        ...
        latest_label = (
            "UPDATE AVAILABLE" if result.update_available else "NEWEST VERSION"
        )
        if status is not None:
            status.setText(f"{latest_label}\nVersion: {result.latest.version}")
        if changelog is not None:
            notes = [result.latest.title, f"Released: {result.latest.released_at}"]
            notes.extend(f"- {item}" for item in result.latest.changelog)

...
def iguanadon_open(metadata):
    ...
    while not inventory.is_open():
        attempt += 1
        logs.logger.debug(
            f"the iguanadon at {metadata.name} could not be accessed retrying {attempt} / {source.gacha_bot.config.iguanadon_attempts}"
        )
        utils.zero()
        ...
        if attempt >= source.gacha_bot.config.iguanadon_attempts:
            logs.logger.error(
                f"the iguanadon at {metadata.name} could not be accesssed after {attempt} attempts"
            )
            break
